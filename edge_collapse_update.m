...
if SHOW_COLLAPSE_PROGRESS || SHOW_RESULTS
    close all;
    figure('Name', 'edge collapse'); movegui('northwest');set(gcf,'color','white');hold on;
...
A(A>0) = 1;
% 消除过长的边
dis = pdist2(spls, spls);
gra = graph(dis); %距离作为边的权重
g = minspantree(gra);% 最小生成树
%plot
% adj_g = adjacency(g);
% figure();
% for i=1:size(adj_g,1)
%     for j=1:size(adj_g,2)
%          if( adj_g(i,j)==1 )
%                idx = [i;j];
%                plot3(spls(idx,1),spls(idx,2),spls(idx,3), 'LineWidth', 2, 'Color', 'b');
%                hold on;
%          end
%      end
% end
...
max_dis = max(g.Edges.Weight);
dis_A = dis.*A;
dis_A(dis_A>max_dis) = 0;
A = dis_A;
A(A>0) = 1;
D = degree(g);
...
while true
    ...
    if collapse_order == 1 % cost is degree + distance
        mind = min( skeds(:,3) ); %skeds: 边的索引和度[i,j,degree,dist]
        tmpIdx = find(skeds(:,3)==mind);%度小的边
    ...
    % update the location 删那个度最小的点
%     if D(edge(1)) < 2 || D(edge(2)) < 2
%         continue;
%     end
    if D(edge(2)) < D(edge(1)) || D(edge(1)) < 2
        m = edge(1);    edge(1) = edge(2);    edge(2) = m;
    else
        direct2 = spls(A(edge(2)) == 1);
        direct2 = direct2 - spls(edge(2));
        direct1 = spls(A(edge(1)) == 1);
        direct1 = direct1 - spls(edge(1));       
        d2 = norm(mean(direct2));
        d1 = norm(mean(direct1));
        if d1 < d2 %端点1在直线上
            m = edge(1);    edge(1) = edge(2);    edge(2) = m;
        end
    end
