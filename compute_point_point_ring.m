function [ring, flag] = compute_point_point_ring(pts, k, index)
...
%     end
    ...
    coefs = pca(neighbor);    
    x = [neighbor * coefs(:, 1), neighbor * coefs(:, 2)];%第一，第二主成分
    
    % 找邻域时，包含索引为1的顶点的一环邻域
    % 序号为1的 最近邻的第一个就是中心顶点自己
    TRI = delaunayn(x, {'QJ','Pp'}); %三角剖分
    if isempty(TRI)
        ring{i} = [];
        continue;
    else
...
%     end
    [row,col] = find(TRI == 1);
    temp = TRI(row,:); %顶点里有1的三角形

    temp = sort(temp,2); %第二维，即把每个三角形内部的顶点排列
    temp = temp(:,2:end); %第二列和第三列
    
    % 找一环的第一个顶点：如果有出现一次的顶点，视为起点，否则任取一个
    x=temp(:);
    x=sort(x);
    d=diff([x;max(x)+1]); %消除相同的编号
    count = diff(find([1;d])); % 每个顶点编号出现的次数
    y =[x(find(d)) count];%出现的编号及其对应的次数
    ...
    neighbor_index = index(i,sorted_index); %index ： 每个点的最近点
    
    % 首位为端点，如果邻域面是封闭的，则首尾数字相同，否则不同
