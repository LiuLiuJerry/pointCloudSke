...
%         PC_COLOR = {0.275, .337, .60, 0.5f}; // blue
        ...
        MIN_NEIGHBOR_NUM = 8;
        MAX_NEIGHBOR_NUM = 10;% orginal 30
        
        LAPLACIAN_CONSTRAINT_WEIGHT = 1; % init Laplacian contraction weight, compute automatically now.
        POSITION_CONSTRAINT_WEIGHT = 6; % init position constraint weight
        POSITION_CONSTRAINT_SCALE = 2; 
        ...
        MIN_POSITION_CONSTRAINT_WEIGHT = 1;
        MAX_CONTRACT_NUM = 5; % max contract iterations 20
        ...
        function ms = one_ring_size(pts,rings,type)
            ...
            switch type
                case 1
                    parfor i = 1:n
                        ring = rings{i};    
                        tmp = repmat(pts(i,:), length(ring),1) - pts(ring,:); %ring 到中心点的距离
                        ms(i) = min( sum(tmp.^2,2).^0.5); %和最近点的距离作为整体的距离
                    end
                case 2
                    parfor i = 1:n
                        ring = rings{i};    
