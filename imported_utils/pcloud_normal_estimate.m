...
function [normals, minCdir, maxCdir] = pcloud_normal_estimate( points, TYPE, k )
...
if strcmp( TYPE, 'hoppe' )
    ...
    minCdir = zeros( size(points,1),3 );
    maxCdir = zeros( size(points,1),3 );
...
%         idxs = kdtree_k_nearest_neighbors( kdtree, p(pIdx,:)', k);
    ...
    TREE = minspantree( tril(ADJ), maxidx );
