function [spls, A, corresp] = edge_collapse_simple(spls, spls_adj, options)
...
for i=1:length(spls)
    ...
    lns = length(ns);
    for j=1:lns %采样后某点的邻居内 相邻的两点，即一个三角形
        for k=j+1:lns
            if A(ns(j),ns(k)) == 1
                tricount = tricount+1;
...
while true
    ...
    if collapse_order == 1 % cost is degree + distance
        mind = min( skeds(:,3) ); %最小的度
        tmpIdx = find(skeds(:,3)==mind); %最小的度的idx
        tmpSkeds = skeds(tmpIdx,4); %最小的距离
        [IGN, idx] = min( tmpSkeds );%最小距离的idx
%         tmp = sortrows(skeds, 4);
%         tmp = sortrows(tmp, 3);
%         tmp = tmp(:, 1:2);
        ...
        [IGN, idx] = min( skeds(:,4) );
        edge = skeds(idx,1:2); %每次都删除距离最小的边的两个点
        skeds(idx,:)=[];
    end
    disp( sprintf('edge to be delete: %d, %d', edge(1),edge(2)) );

    % update the location
    if ~contains(D_id, edge(1))
        spls( edge(1) ) = [];
    end
